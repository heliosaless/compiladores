...
public class Codegen2 {
    ...
    public Codegen2(Frame f, ListIterator<Assem.Instr> li) {
        frame = f;
        this.li = li;
    }
    ...
    void munchStm(Stm s) {
        if (s instanceof SEQ) {
            ...
            munchCJump((CJUMP) s);
        else if (s instanceof EXP1 && ((EXP1) s).exp instanceof CALL)
            munchCall(((CALL) ((EXP1) s).exp));
        else {
    ...
    Translation.Temp.Temp munchExp(Exp s) {
        if (s instanceof MEM)
            return munchExpMEM((MEM) s);
        else if (s instanceof BINOP)
            return munchExpBINOP((BINOP) s);
        else if (s instanceof CONST)
            return munchExpCONST((CONST) s);
        else if (s instanceof CALL)
            return munchExpCall(((CALL) s));
        else if (s instanceof NAME)
            return munchExpNAME((NAME) s);
    ...
    Translation.Temp.Temp munchExpCONST(CONST s) {
        Temp r = new Temp();
        emit(new Assem.OPER("ADDI `d0 <- r0+" + s.value + "\n", new TempList(r, null), null));
        return r;
    }

    Translation.Temp.Temp munchExpMEM(MEM s) {
        if (s.exp instanceof BINOP && ((BINOP) s.exp).binop == BINOP.PLUS && ((BINOP) s.exp).right instanceof CONST) {
            Temp r = new Temp();
            int i = ((CONST) ((BINOP) s.exp).right).value;
            emit(new Assem.OPER("LOAD `d0 <- M[`s0+" + i + "]\n", new TempList(r, null),
            ...
            emit(new Assem.OPER("LOAD `d0 <- M[`s0+" + i + "]\n", new TempList(r, null),
                    new TempList(munchExp(((BINOP) s.exp).right), null)));
            return r;
        }
            ...
            emit(new Assem.OPER("LOAD `d0 <- M[r0+" + i + "]\n", new TempList(r, null), null));
            return r;
        }
    ...
    Translation.Temp.Temp munchExpBINOP(BINOP s) {
        if (s.binop == BINOP.PLUS && s.right instanceof CONST) {
            ...
            emit(new Assem.OPER("ADDI `d0 <- `s0+" + i + "\n", new TempList(r, null),
                    new TempList(munchExp(s.left), null)));
            return r;
        }
        ...
        else if (s.binop == BINOP.MINUS && s.right instanceof CONST) {
            Temp r = new Temp();
            int i = ((CONST) s.right).value;
            emit(new Assem.OPER("SUBI `d0 <- `s0+" + i + "\n", new TempList(r, null),
                    new TempList(munchExp(s.left), null)));
            return r;
        }
    ...
    void munchJump(JUMP s) {
        ...
        emit(new Assem.OPER("JUMP `j0 " + "\n", null, new TempList(aux, null), s.targets));
    }
    ...
    void munchMove(TEMP dst, Exp src) {
        if (src instanceof TEMP) {
            emit(new Assem.MOVE("MOVE `d0 <- $`s0", dst.temp, ((TEMP) src).temp));
        } else
            emit(new Assem.OPER("ADD `d0 <- `s0 + r0", new TempList(dst.temp, null),
    ...
    void munchMove(MEM dst, Exp src) {
        ...
        if (dst.exp instanceof BINOP && ((BINOP) dst.exp).binop == BINOP.PLUS
            ...
            emit(new Assem.OPER("STORE M[`s0+" + i + "] <- `s1\n", null, new TempList(dst_, new TempList(src_, null))));
        }
        ...
        //     Temp src_ = munchExp(((MEM) src).exp);
        //     emit(new Assem.OPER("MOVE M[`s0] <- M[`s1]\n", null, new TempList(dst_, new TempList(src_, null))));
        // }

        else if (dst.exp instanceof CONST) {
            Temp src_ = munchExp(src);
            int i = ((CONST) dst.exp).value;
            emit(new Assem.OPER("STORE M[r0+" + i + "] <- `s0\n", null, new TempList(src_, null)));
        }
        ...
        // MOVE(MEM(e1), e2)
            ...
            Temp dst_ = munchExp(dst.exp);
            Temp src_ = munchExp(src);
            emit(new Assem.OPER("STORE M[`s0] <- `s1\n", null, new TempList(dst_, new TempList(src_, null))));
        }
