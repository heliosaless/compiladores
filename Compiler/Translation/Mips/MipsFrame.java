...
public class MipsFrame extends Frame {
    ...
    public Translation.Tree.Exp externalCall(String s, Translation.Tree.ExpList args) {
	...
	// args.add(0, new Translation.Tree.CONST(0));
	args = new Translation.Tree.ExpList(new Translation.Tree.CONST(0), args);
	return new Translation.Tree.CALL(new Translation.Tree.NAME(l), args);
    }
	...
	if (right == null)
	    return left;
	return new Translation.Tree.SEQ(left, right);
    }
    private static Translation.Tree.MOVE MOVE(Translation.Tree.Exp d, Translation.Tree.Exp s) {
	return new Translation.Tree.MOVE(d, s);
    }
    private static Translation.Tree.TEMP TEMP(Temp t) {
	return new Translation.Tree.TEMP(t);
    }

    private void
	assignFormals(Iterator<Access> formals,
		      Iterator<Access> actuals,
		      List<Translation.Tree.Stm> body)
    {
    ...
    private void assignCallees(int i, List<Translation.Tree.Stm> body)
    {
    ...
    public void procEntryExit1(List<Translation.Tree.Stm> body) {
	if(formals != null)
	...
	// 	Translation.Tree.Exp exp = allocLocal(true).exp(TEMP(FP));
	...
	// 			Temp t = new Temp();
	// 			t.spillTemp = true;
	// 			Translation.Tree.Stm stm = MOVE(TEMP(t), exp);
	// 			i.previous();
	...
	// 			Translation.Tree.Stm stm = MOVE(exp, TEMP(t));
	// 			stm.accept(new Codegen(this, i));
	// 			break;
	// 		    }
	// 		}
